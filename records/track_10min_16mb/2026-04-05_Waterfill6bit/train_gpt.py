...
class Hyperparameters:
    ...
    # Waterfill-6bit quantization configuration
        ...
        if self.training and w.ndim == 2:
            if CastedLinear._waterfill_enabled:
                ...
                with torch.no_grad():
                    ...
                    scale = (row_max / 31.0).clamp_min(1e-6)
...
def _waterfill_quantize_weight(weight: Tensor, activations: Tensor | None) -> dict:
    """
    Waterfill-6bit: post-training quantization of one weight matrix.

    Algorithm:
      1. Compute per-column input-energy h_i = sqrt(mean(x[:,i]^2)) from calibration activations.
      2. Waterfilling column scale: s_i = col_range_i / (h_norm_i * 31).
         High-activation columns get finer resolution (smaller scale), reducing output error.
      3. Quantize: Z = clamp(round(W / s), -32, 31) — 64 levels, signed 6-bit.
      4. Reconstruct W_approx = Z * s — no L matrix at inference.
      5. Store Z as int8 (lzma/zstd compresses the 2 unused bits) + fp16 per-column scales.

    Returns dict with keys: 'Z' (int8), 'col_scale' (fp16), 'shape' (tuple).
    """
    clip = 31  # 6-bit signed: levels -32 … +31
    w32 = weight.float()
    out_dim, in_dim = w32.shape

    if activations is not None and activations.numel() > 0:
        act = activations.float()  # [samples, in_dim]
        h = act.pow(2).mean(dim=0).clamp_min(1e-8).sqrt()  # RMS per column
        col_range = w32.abs().amax(dim=0).clamp_min(1e-6)
        # Finer resolution for high-activation columns (waterfilling)
...
def _waterfill_dequantize_weight(d: dict) -> Tensor:
    """Reconstruct fp32 weight from waterfill-6bit storage."""
    Z = d["Z"]  # [out, in] int8
    col_scale = d["col_scale"].float()  # [in]
    return Z.float() * col_scale[None, :]


def mixed_quantize_waterfill6(state_dict: dict[str, Tensor],
                              quant_cats: set[str],
                              calibration_data: dict[str, Tensor] | None = None):
    """Quantize attention + MLP weights to 6-bit waterfill (per-column scale); rest to int8."""
...
def dequantize_mixed_waterfill6(result: dict, meta: dict,
                                template_sd: dict[str, Tensor]) -> dict[str, Tensor]:
    ...
    for name, orig in template_sd.items():
        ...
        if info is None:
            continue
        orig_dtype = orig.dtype
        if isinstance(info, dict) and info.get("type") == "waterfill6":
...
def main() -> None:
    ...
    while True:
        ...
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not CastedLinear._qat_enabled:
            ...
            log0(f"late_qat:enabled step:{step} scale:{scale:.4f} waterfill6:{args.waterfill_enabled}")
        zero_grad_all()
    ...
    if args.waterfill_enabled:
        log0("Collecting calibration activations for waterfill-6bit...")
        calibration_data = _get_calibration_activations(model, val_tokens, args.waterfill_calibration_samples)
        log0("Applying waterfill-6bit quantization...")
        quant_result, quant_meta = mixed_quantize_waterfill6(unbanked_sd, {"mlp", "attn"}, calibration_data)
        quant_buf = io.BytesIO()
        torch.save({"w": quant_result, "m": quant_meta}, quant_buf)
        quant_raw = quant_buf.getvalue()
        quant_blob = _compress(quant_raw)
        if master_process:
            with open("final_model.wf6.ptz", "wb") as f:
                f.write(quant_blob)
            quant_file_bytes = len(quant_blob)
            code_bytes = len(code.encode("utf-8"))
            log0(f"Serialized model wf6+{_COMPRESSOR}: {quant_file_bytes} bytes")
            log0(f"Total submission size wf6+{_COMPRESSOR}: {quant_file_bytes + code_bytes} bytes")
        if distributed:
            dist.barrier()
        with open("final_model.wf6.ptz", "rb") as f:
            quant_blob_disk = f.read()
        quant_state = torch.load(io.BytesIO(_decompress(quant_blob_disk)), map_location="cpu")
        deq_unbanked = dequantize_mixed_waterfill6(quant_state["w"], quant_state["m"], unbanked_sd)
    else:
        # int6 per-row quantization (fallback)
    ...
    torch.cuda.synchronize()
    log0(
        f"final_{'wf6' if args.waterfill_enabled else 'int6'}_roundtrip val_loss:{q_val_loss:.4f} val_bpb:{q_val_bpb:.4f} "
        f"eval_time:{1000.0 * (time.perf_counter() - t_qeval):.0f}ms"
    )
    log0(f"final_{'wf6' if args.waterfill_enabled else 'int6'}_roundtrip_exact val_loss:{q_val_loss:.8f} val_bpb:{q_val_bpb:.8f}")
    ...
    if args.eval_stride > 0 and args.eval_stride < sw_seq_len:
        ...
        torch.cuda.synchronize()
        log0(
            f"final_{'wf6' if args.waterfill_enabled else 'int6'}_sliding_window val_loss:{sw_val_loss:.4f} val_bpb:{sw_val_bpb:.4f} "
            f"stride:{args.eval_stride} eval_time:{1000.0 * (time.perf_counter() - t_slide):.0f}ms"
        )
        log0(f"final_{'wf6' if args.waterfill_enabled else 'int6'}_sliding_window_exact val_loss:{sw_val_loss:.8f} val_bpb:{sw_val_bpb:.8f}")
