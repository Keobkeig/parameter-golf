...
class Hyperparameters:
    ...
    # Waterfill-4bit quantization configuration
    waterfill_enabled = bool(int(os.environ.get("WATERFILL_ENABLED", "0")))
    waterfill_calibration_samples = int(os.environ.get("WATERFILL_CALIBRATION_SAMPLES", 512))
...
class CastedLinear(nn.Linear):
    ...
    _waterfill_enabled: bool = False

    def forward(self, x: Tensor) -> Tensor:
        w = self.weight.to(x.dtype)
        if self.training and w.ndim == 2:
            if CastedLinear._waterfill_enabled:
                # 4-bit waterfill fake quantization: per-column scale STE
                with torch.no_grad():
                    w32 = self.weight.float()
                    col_scale = w32.abs().amax(dim=0).clamp_min(1.0 / 8.0)  # [in]
                    w_norm = w32 / col_scale[None, :]
                    w_q = (torch.clamp(torch.round(w_norm * 7.0), -8.0, 7.0) / 7.0 * col_scale[None, :]).to(x.dtype)
                w = w + (w_q - w).detach()
            elif CastedLinear._qat_enabled:
                # int6 fake quantization (kept for reference / fallback)
                with torch.no_grad():
                    w32 = self.weight.float()
...
# --- Waterfill-4bit Quantization ---

def _get_calibration_activations(model: nn.Module, val_tokens: Tensor, max_samples: int = 512) -> dict[str, Tensor]:
...
def _waterfill_quantize_weight(weight: Tensor, activations: Tensor | None) -> dict:
    """
    Waterfill-4bit: post-training quantization of one weight matrix.

    Algorithm:
      1. Compute per-column input-energy h_i = mean(x[:,i]^2) from calibration activations.
      2. Build waterfilling column scales: s_i = sqrt(h_i) * ||w[:,i]||_inf / clip_range.
         (Allocates more resolution to columns that carry more signal.)
      3. Quantize in the normalized column space: Z = clamp(round(W / s), -8, 7).
      4. Reconstruct W_approx = Z * s  — no L matrix needed at inference.
      5. Pack Z as 4-bit (2 values per byte) and store s as fp16.

    Returns dict with keys: 'packed' (uint8), 'col_scale' (fp16), 'shape' (tuple).
    """
    clip = 7  # 4-bit signed: levels -8 … +7
    w32 = weight.float()
    out_dim, in_dim = w32.shape

    # Per-column scale: waterfilling-inspired (activation energy × weight range)
    ...
    # Pack 4-bit: two int4 values per byte (low nibble = even cols, high nibble = odd cols)
    # Pad in_dim to even if needed
    pad = (in_dim % 2 != 0)
    if pad:
        Z = F.pad(Z, (0, 1))
    Z_u = (Z + 8).to(torch.uint8)  # shift to [0, 15]
    packed = (Z_u[:, 0::2] & 0xF) | ((Z_u[:, 1::2] & 0xF) << 4)  # [out, in//2]

    return {
        "packed": packed.contiguous(),
        "col_scale": col_scale_f16.contiguous(),
        "shape": (out_dim, in_dim),
    }


def _waterfill_dequantize_weight(d: dict) -> Tensor:
    """Reconstruct fp32 weight from waterfill-4bit storage."""
    clip = 7
    packed = d["packed"]  # [out, in//2] uint8
    col_scale = d["col_scale"].float()  # [in_padded]
    out_dim, in_dim = d["shape"]

    low = (packed & 0xF).to(torch.int8) - 8   # [out, in//2]
    high = ((packed >> 4) & 0xF).to(torch.int8) - 8
    Z = torch.zeros(out_dim, packed.shape[1] * 2, dtype=torch.int8)
    Z[:, 0::2] = low
    Z[:, 1::2] = high
    Z = Z[:, :in_dim]  # remove padding if any
    col_scale = col_scale[:in_dim]
    return Z.float() * col_scale[None, :]


def mixed_quantize_waterfill4(state_dict: dict[str, Tensor],
                              quant_cats: set[str],
                              calibration_data: dict[str, Tensor] | None = None):
    """Quantize attention + MLP weights to 4-bit waterfill; rest to int8."""
    num_layers_total = max(
        int(k.split(".")[1]) for k in state_dict if k.startswith("blocks.") and k.split(".")[1].isdigit()
    ) + 1
    ...
    for name, t in state_dict.items():
        cat = _classify_param(name)
        if cat in quant_cats and t.ndim == 2 and t.numel() > 65536:
            acts = calibration_data.get(name) if calibration_data else None
            d = _waterfill_quantize_weight(t.detach().cpu(), acts)
            result[name] = d
            meta[name] = {"type": "waterfill4"}
        else:
            q, s = quantize_float_tensor(t.detach().cpu().contiguous())
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int8"}
...
def dequantize_mixed_waterfill4(result: dict, meta: dict,
                                template_sd: dict[str, Tensor]) -> dict[str, Tensor]:
    ...
    for name, orig in template_sd.items():
        ...
        if info is None:
            continue
        orig_dtype = orig.dtype
        if isinstance(info, dict) and info.get("type") == "waterfill4":
            w = _waterfill_dequantize_weight(result[name])
            dequantized[name] = w.to(orig_dtype)
...
# ---- legacy stubs (removed, replaced by waterfill above) ----
...
def main() -> None:
    ...
    CastedLinear._waterfill_enabled = args.waterfill_enabled and args.qat_enabled
    base_model = GPT(
    ...
    while True:
        ...
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not CastedLinear._qat_enabled:
            ...
            CastedLinear._waterfill_enabled = args.waterfill_enabled
            log0(f"late_qat:enabled step:{step} scale:{scale:.4f} waterfill:{args.waterfill_enabled}")
        zero_grad_all()
    ...
    if args.waterfill_enabled:
        # Collect calibration activations for waterfilling scales
        log0("Collecting calibration activations for waterfill-4bit...")
        calibration_data = _get_calibration_activations(model, val_tokens, args.waterfill_calibration_samples)
        log0("Applying waterfill-4bit quantization...")
        quant_result, quant_meta = mixed_quantize_waterfill4(unbanked_sd, {"mlp", "attn"}, calibration_data)
        quant_buf = io.BytesIO()
        torch.save({"w": quant_result, "m": quant_meta}, quant_buf)
        quant_raw = quant_buf.getvalue()
        quant_blob = lzma.compress(quant_raw, preset=6)
        if master_process:
            with open("final_model.wf4.ptz", "wb") as f:
                f.write(quant_blob)
            quant_file_bytes = len(quant_blob)
            code_bytes = len(code.encode("utf-8"))
            log0(f"Serialized model wf4+lzma: {quant_file_bytes} bytes")
            log0(f"Total submission size wf4+lzma: {quant_file_bytes + code_bytes} bytes")
        if distributed:
            dist.barrier()
        with open("final_model.wf4.ptz", "rb") as f:
            quant_blob_disk = f.read()
        quant_raw_disk = lzma.decompress(quant_blob_disk)
        quant_state = torch.load(io.BytesIO(quant_raw_disk), map_location="cpu")
        deq_unbanked = dequantize_mixed_waterfill4(quant_state["w"], quant_state["m"], unbanked_sd)
    else:
        # int6 quantization (baseline)
    ...
    torch.cuda.synchronize()
    log0(
        f"final_{'wf4' if args.waterfill_enabled else 'int6'}_roundtrip val_loss:{q_val_loss:.4f} val_bpb:{q_val_bpb:.4f} "
        f"eval_time:{1000.0 * (time.perf_counter() - t_qeval):.0f}ms"
    )
    log0(f"final_{'wf4' if args.waterfill_enabled else 'int6'}_roundtrip_exact val_loss:{q_val_loss:.8f} val_bpb:{q_val_bpb:.8f}")
    ...
    if args.eval_stride > 0 and args.eval_stride < sw_seq_len:
        ...
        torch.cuda.synchronize()
        log0(
            f"final_{'wf4' if args.waterfill_enabled else 'int6'}_sliding_window val_loss:{sw_val_loss:.4f} val_bpb:{sw_val_bpb:.4f} "
            f"stride:{args.eval_stride} eval_time:{1000.0 * (time.perf_counter() - t_slide):.0f}ms"
        )
        log0(f"final_{'wf4' if args.waterfill_enabled else 'int6'}_sliding_window_exact val_loss:{sw_val_loss:.8f} val_bpb:{sw_val_bpb:.8f}")
